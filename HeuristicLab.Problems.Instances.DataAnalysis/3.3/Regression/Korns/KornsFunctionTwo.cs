...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class KornsFunctionTwo : ArtificialRegressionDataDescriptor {

    public override string Name { get { return "Korns 2 y = 0.23 + (14.2 * ((X3 + X1) / (3.0 * X4)))"; } }
    ...
    public KornsFunctionTwo() : this((int)System.DateTime.Now.Ticks) {
    }
    public KornsFunctionTwo(int seed) : base() {
      Seed = seed;
    }
    protected override List<List<double>> GenerateValues() {
      ...
      for (int i = 0; i < data[0].Count; i++) {
        ...
        x3 = data[3][i];
        x4 = data[4][i];
        results.Add(0.23 + (14.2 * ((x3 + x1) / (3.0 * x4))));
      }
      data.Add(results);
