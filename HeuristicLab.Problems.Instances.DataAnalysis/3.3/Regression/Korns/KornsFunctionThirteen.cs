...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class KornsFunctionThirteen : ArtificialRegressionDataDescriptor {

    public override string Name { get { return "Korns 13 y = 32.0 - (3.0 * ((tan(X0) / tan(X1)) * (tan(X2) / tan(X3))))"; } }
    ...
    public KornsFunctionThirteen() : this((int)System.DateTime.Now.Ticks) {
    }
    public KornsFunctionThirteen(int seed) : base() {
      Seed = seed;
    }
    protected override List<List<double>> GenerateValues() {
      ...
      for (int i = 0; i < data[0].Count; i++) {
        ...
        x2 = data[2][i];
        x3 = data[3][i];
        results.Add(32.0 - (3.0 * ((Math.Tan(x0) / Math.Tan(x1)) * (Math.Tan(x2) / Math.Tan(x3)))));
      }
      data.Add(results);
