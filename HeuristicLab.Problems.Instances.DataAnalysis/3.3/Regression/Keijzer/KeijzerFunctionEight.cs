...
namespace HeuristicLab.Problems.Instances.DataAnalysis {
  public class KeijzerFunctionEight : ArtificialRegressionDataDescriptor {
    ...
    protected override List<List<double>> GenerateValues() {
      ...
      data.Add(SequenceGenerator.GenerateSteps(0m, 100, 1).Select(v => (double)v).ToList());
      data[0].AddRange(SequenceGenerator.GenerateSteps(0m, 100, 0.1m).Select(v => (double)v));
