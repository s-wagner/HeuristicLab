      ...
      foreach (object key in transformers.Keys) {
        ...
        if (r.Equals(Rectangle.Empty)) //the bone represents a connection
          ...
          if (!bone.ConnectorPoints[0].Equals(Point.Empty)) {
            ...
            conn.From.MoveBy(new Point(x, y));
          }
          ...
          x = Convert.ToInt32(Math.Round(
              (r.X - origin.X) * scalex + origin.X, 1));

          y = Convert.ToInt32(Math.Round(
              (r.Y - origin.Y) * scaley + origin.Y, 1));

          w = Convert.ToInt32(r.Width * scalex);

          h = Convert.ToInt32(r.Height * scaley);

          // Only perform the tranform if the rectangle's height 
          // and width are less than or equal to the shape's  
          // min/max size.
          if ((w <= shape.MaxSize.Width) &&
              (h <= shape.MaxSize.Height) &&
              (w >= shape.MinSize.Width) &&
              (h >= shape.MinSize.Height)) {
            shape.Transform(x, y, w, h);
          }
