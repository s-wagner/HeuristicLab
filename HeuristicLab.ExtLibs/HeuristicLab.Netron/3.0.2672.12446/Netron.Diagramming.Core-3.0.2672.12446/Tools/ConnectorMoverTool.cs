...
namespace Netron.Diagramming.Core {
  ...
  class ConnectorMoverTool : AbstractTool, IMouseListener {
    ...
    public ConnectorMoverTool(string name)
      : base(name) {
    }
    #endregion
    ...
    protected override void OnActivateTool() {
      Controller.View.CurrentCursor = CursorPalette.Select;
      motionStarted = false;
    ...
    public void MouseMove(MouseEventArgs e) {
      ...
      if (IsActive && motionStarted) {
        ...
        fetchedConnector.MoveBy(new Point(point.X - lastPoint.X, point.Y - lastPoint.Y));
