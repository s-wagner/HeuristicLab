...
namespace Netron.Diagramming.Core {
  // ----------------------------------------------------------------------
  /// <summary>
  /// This is an extension of the <see cref="GroupShape"/> which paints 
  /// itself and allows to collapse/expand its content. Note that it does 
  /// not inherit from the <see cref="GroupShape"/> though because it requires 
  /// the <see cref="ShapeMaterialBase"/> mechanism and, hence, inherits 
  /// from the <see cref="ComplexShapeBase"/> class.
  /// </summary>
  // ----------------------------------------------------------------------
  public partial class CollapsibleGroupShape : ComplexShapeBase, IGroup {
    #region Fields

    // ------------------------------------------------------------------
    /// <summary>
    /// Implementation of IVersion - the current version of 
    /// CollapsibleGroupShape.
    /// </summary>
    // ------------------------------------------------------------------
    protected const double collapsibleGroupShapeVersion = 1.0;
    ...
    public override double Version {
      get {
        return collapsibleGroupShapeVersion;
      }
    }
    ...
    private void MoveConnector(IConnector cn, Point point) {
      ...
      cn.MoveBy(new Point(point.X - cn.Point.X, point.Y - cn.Point.Y));
      cn.Enabled = false;
    }
    ...
    private void UnMoveConnectors() {
      ...
      foreach (IConnector cn in keys) {
        ...
        cn.MoveBy(new Point(p.X - cn.Point.X + groupConnectorOffset.X, p.Y - cn.Point.Y + groupConnectorOffset.Y));
        cn.Enabled = true;
      }
    }
    ...
    public override void Paint(System.Drawing.Graphics g) {
      ...
      Utils.DrawRoundRect(g, ArtPalette.ConnectionShadow, rec);
    ...
    /// </summary>
    /// <param name="p"></param>
    public override void MoveBy(System.Drawing.Point p) {
      base.MoveBy(p);
    ...
    public override bool MouseDown(MouseEventArgs e) {
      // Was a material hit in the base ComplexShape?
      if (base.MouseDown(e)) {
        return true;
      }

      // If not, should we pass it on to the entities in this
      // group?
      foreach (IDiagramEntity entity in this.mEntities) {
