...
namespace HeuristicLab.Services.OKB.Query.Filters {
  public class LongValueFilter : IFilter {
      ...
      get {
        switch (Comparison) {
          case DataTransfer.OrdinalComparison.Less:
            return x => x.Values.Any(y => (y.ValueName.Name == Name) && (y.LongValue < Value));
    ...
    public LongValueFilter(string name, DataTransfer.OrdinalComparison comparison, long value) {
      Name = name;
      Comparison = comparison;
      Value = value;
    }
    public LongValueFilter(DataTransfer.NameOrdinalComparisonLongFilter filter) {
      Name = filter.Name;
