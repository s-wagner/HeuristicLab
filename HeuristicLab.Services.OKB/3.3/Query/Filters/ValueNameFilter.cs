...
namespace HeuristicLab.Services.OKB.Query.Filters {
  public class ValueNameFilter : IFilter {
      ...
      get {
        switch (Comparison) {
          case DataTransfer.StringComparison.Equal:
            return x => x.Values.Any(y => y.ValueName.Name == Value);
          case DataTransfer.StringComparison.NotEqual:
            return x => x.Values.All(y => y.ValueName.Name != Value);
          case DataTransfer.StringComparison.Contains:
            return x => x.Values.Any(y => y.ValueName.Name.Contains(Value));
          case DataTransfer.StringComparison.NotContains:
            return x => x.Values.All(y => !y.ValueName.Name.Contains(Value));
          case DataTransfer.StringComparison.Like:
            return x => x.Values.Any(y => SqlMethods.Like(y.ValueName.Name, Value));
          case DataTransfer.StringComparison.NotLike:
            return x => x.Values.All(y => !SqlMethods.Like(y.ValueName.Name, Value));
          default:
            return x => true;
        }
      }
